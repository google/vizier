...
class GaussianProcessARDWithCategorical(sp_model.ModelCoroutine):
  ...
  gen = GaussianProcessARDWithCategorical(
      dimension=tfpks.ContinuousAndCategoricalValues(
  ...
  def __call__(
      self, inputs: Optional[tfpks.ContinuousAndCategoricalValues] = None
  ) -> Generator[sp_model.ModelParameter, Array, tfd.GaussianProcess]:
    # pylint: disable=g-doc-return-or-yield
    """The coroutine that specifies the GP model.

    The inputs (and the index points of the returned Gaussian process) are
    instances of `tfpks.ContinuousAndCategoricalValues`, a `namedtuple` with
    ...
    kernel = tfpks.FeatureScaledWithCategorical(
        kernel,
        inverse_scale_diag=tfpks.ContinuousAndCategoricalValues(
