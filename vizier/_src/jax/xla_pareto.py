...
def is_frontier(
  ...
  """
  idx = np.linspace(0, ys.shape[0], num_shards).astype(np.int32)  # pytype: disable=attribute-error
  idx = list(reversed(idx))
  # Initialize candidates with all points
  frontier = np.ones(ys.shape[0], dtype=np.bool_)  # pytype: disable=attribute-error
