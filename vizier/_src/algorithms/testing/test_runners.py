...
# TODO: Integrate this into benchmarks/, or algorithms/core
...
class _AlgorithmRunnerProtocol(Protocol):
  ...
  def post_completion_callback(self, completed: vza.CompletedTrials) -> None:
    """Callback after the suggestions are completed."""


@attr.define
class _DesignerRunnerProtocol(_AlgorithmRunnerProtocol):
  """Wraps a Designer into the RunnerProtocol.

  Designers return Suggestions which don't have ids assigned to them. We use
  LocalPolicyRunner to manage Trial ids.
  """
  _designer: vza.Designer = attr.field()
  _runner: pythia.LocalPolicyRunner = attr.field()

  def suggest(self, batch_size: Optional[int]) -> Collection[vz.Trial]:
    suggestions: Collection[vz.TrialSuggestion] = self._designer.suggest(
        batch_size)
    # Assign ids.
    trials: Collection[vz.Trial] = self._runner.AddSuggestions(suggestions)
    return trials
...
@attr.define
class _PolicyRunnerProtocol(_AlgorithmRunnerProtocol):
  """Wraps a Policy into the RunnerProtocol."""
  _policy: pythia.Policy = attr.field()
  _runner: pythia.LocalPolicyRunner = attr.field()

  def suggest(self, batch_size: Optional[int]) -> Collection[vz.Trial]:
    return self._runner.SuggestTrials(self._policy, count=batch_size or 1)
...
class RandomMetricsRunner:
  ...
  def _run(self, algorithm: _AlgorithmRunnerProtocol) -> Collection[vz.Trial]:
  ...
  def run_policy(self, policy_factory: Callable[[pythia.PolicySupporter],
    ...
    runner = pythia.LocalPolicyRunner(self.problem)
    policy = policy_factory(runner)
    return self._run(_PolicyRunnerProtocol(policy, runner))

  def run_designer(self, designer: vza.Designer):
    """Run the specified $designer."""
    runner = pythia.LocalPolicyRunner(self.problem)
    return self._run(_DesignerRunnerProtocol(designer, runner))
